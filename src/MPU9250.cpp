...
void MPU9250::setupMagForSPI()
{
  ...
  writeByteSPI(I2C_SLV4_ADDR, ((1 << 7) | AK8963_ADDRESS));    // Set the SLV_4_ADDR register to the magnetometer's address
...
void MPU9250::readMagData(int16_t * destination)
{
  ...
  // Check if data are deady
  if (readByte(AK8963_ADDRESS, AK8963_ST1) & 0x01)
  {
    // Read measurent data with ST2 register
...
//call after AK8963GetSensivity
bool MPU9250::AK8963SelfTest()
{
  uint8_t rawData[6] = {0};
  

  writeByte(AK8963_ADDRESS, AK8963_CNTL, 0x0); //Power down magnetometer
  writeByte(AK8963_ADDRESS, AK8963_ASTC, 0x40); //0b01000000 Generate magnetic field for selftest
  writeByte(AK8963_ADDRESS, AK8963_CNTL, 0x18); //0b00011000 Set selftest mode with 16bit output
  delay(10);

  rawData[0] = readByte(AK8963_ADDRESS, AK8963_ST1);
  if (rawData[0] == 0x1)
  {
    readBytes(AK8963_ADDRESS, AK8963_XOUT_L, 7, &rawData[0]);
    magSTX = (((int16_t)rawData[1]) << 8) | rawData[0];
    magSTY = (((int16_t)rawData[3]) << 8) | rawData[2];
    magSTZ = (((int16_t)rawData[5]) << 8) | rawData[4];
  }

  writeByte(AK8963_ADDRESS, AK8963_ASTC, 0x0); // Turn off generating magnetic field

  if (magSTX * magSensX >= -200 && magSTX * magSensX <= 200 &&
      magSTY * magSensY >= -200 && magSTY * magSensY <= 200 &&
      magSTZ * magSensZ >= -3200 && magSTZ * magSensZ <= -800)
  {
    return true;
  }
  else
...
void MPU9250::AK8963GetSensivity()
{
  uint8_t rawData[3] = {0};
  ...
  // Read ASAX, ASAY, ASAZ sensitivity adjustment values 
  readBytes(AK8963_ADDRESS, AK8963_ASAX, 3, &rawData[0]);

  // Return x, y, z axis sensitivity adjustment values
  magSensX =  (uint8_t)(rawData[0] - 128)/256 + 1;
  magSensY =  (uint8_t)(rawData[1] - 128)/256 + 1;
  magSensZ =  (uint8_t)(rawData[2] - 128)/256 + 1;
...
void MPU9250::AK8963Mode(uint8_t mscale, uint8_t mmode) 
{
  ...
  if(_csPin != NOT_SPI)
  {
    setupMagForSPI(); //set magnetometer as SLV_4 and multimaster mode
  }
}

void MPU9250::initMPU9250()
{
  if (readByte(_I2Caddr, WHO_AM_I_MPU9250) == 0x71)
  {
  // wake up device Clear sleep mode bit (6), enable all sensors
  writeByte(_I2Caddr, PWR_MGMT_1, 0x0);
  
  delay(100);

  // Auto select clock source,if ready it is PLL gyroscope reference else internal oscillator 
...
void MPU9250::calibrateMPU9250(float * gyroBias, float * accelBias)
{
  ...
  // Divide summed biases by packet_count to get accumulated signed 32-bit biases
...
// deviation is a pass. Call after setting biases
void MPU9250::MPU9250SelfTest(float * destination)
{
  uint8_t rawData[6] = {0, 0, 0, 0, 0, 0};
  int32_t gAvg[3] = {0}, aAvg[3] = {0}, aSTAvg[3] = {0}, gSTAvg[3] = {0};
  float factoryTrim[6];
  uint8_t FS = 0x0; // Select scale 250dps for gyro and 2g for accel
  ...
  // read 204 values of gyro and acclerometer without selftest
  ...
  // Get average of 204 values and store as average current readings
  ...
  // Get average of 204 values and store as average self-test readings
  for (int ii =0; ii < 3; ii++)
  {
    aSTAvg[ii] /= 204;
    gSTAvg[ii] /= 204;
  ...
  factoryTrim[0] = (float)(2620<<FS)*(pow(1.01 ,((float)selfTest[0] - 1.0) ));
  // FT[Ya] factory trim calculation
  factoryTrim[1] = (float)(2620<<FS)*(pow(1.01 ,((float)selfTest[1] - 1.0) ));
  // FT[Za] factory trim calculation
  factoryTrim[2] = (float)(2620<<FS)*(pow(1.01 ,((float)selfTest[2] - 1.0) ));
  // FT[Xg] factory trim calculation
  factoryTrim[3] = (float)(2620<<FS)*(pow(1.01 ,((float)selfTest[3] - 1.0) ));
  // FT[Yg] factory trim calculation
  factoryTrim[4] = (float)(2620<<FS)*(pow(1.01 ,((float)selfTest[4] - 1.0) ));
  // FT[Zg] factory trim calculation
  factoryTrim[5] = (float)(2620<<FS)*(pow(1.01 ,((float)selfTest[5] - 1.0) ));
...
/*
  if ((aSTAvg[0] - aAvg[0]) / factoryTrim[0] < 1.5 &&
      (aSTAvg[0] - aAvg[0]) / factoryTrim[0] > 0.5 &&

      (aSTAvg[1] - aAvg[1]) / factoryTrim[1] < 1.5 &&
      (aSTAvg[0] - aAvg[1]) / factoryTrim[1] > 0.5 &&

      (aSTAvg[2] - aAvg[2]) / factoryTrim[2] < 1.5 &&
      (aSTAvg[0] - aAvg[2]) / factoryTrim[2] > 0.5 &&

      (gSTAvg[0] - gAvg[0])/factoryTrim[3] > 0.5 &&
      (gSTAvg[1] - gAvg[1])/factoryTrim[4] > 0.5 &&
      (gSTAvg[2] - gAvg[2])/factoryTrim[5] > 0.5)
  {
...
uint8_t MPU9250::readMagByteSPI(uint8_t registerAddress)
{
  ...
  uint32_t count = 0;
  while(((I2C_MASTER_STATUS & 0b01000000) == 0) && (count++ < 100000))            // Bez sensu wysyla zapytania jak pojebane Checks against the I2C_SLV4_DONE bit in the I2C master status register
  {
...
uint8_t MPU9250::writeMagByteSPI(uint8_t registerAddress, uint8_t data)
{
  ...
  writeByteSPI(49, ((1 << 7) | AK8963_ADDRESS)); //trzeba zmienic na 0 7bit zeby bylo write
  writeByteSPI(50, registerAddress);
...
uint8_t MPU9250::readBytesSPI(uint8_t registerAddress, uint8_t count, uint8_t * dest)
{
